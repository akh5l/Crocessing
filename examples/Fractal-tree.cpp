...
void drawBranch(float x1, float y1, float angle, float length, int depth, std::vector<Branch> &branches) {
    ...
    drawBranch(x2, y2, angle - M_PI / 6.0f, length * 0.7f, depth - 1, branches);  // Left branch
    drawBranch(x2, y2, angle + M_PI / 6.0f, length * 0.7f, depth - 1, branches);  // Right branch
}
...
void CSketch::setup() {
    ...
    drawBranch(x1, y1, angle, length, depth, branches);
}
